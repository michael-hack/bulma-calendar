...
gulp.task("clean:scripts", function () {
  return del([
    ...
    config.javascript.destination +
      config.javascript.output.filename +
      ".min.js",
  ]);
});
...
gulp.task(
  ...
  gulp.series("build", function () {
    ...
    gulp.watch(
      config.javascript.source + paths.pattern.js,
      gulp.series("build:scripts", "copy:scripts", "sync")
    );
    gulp.watch(
      config.sass.source + paths.pattern.sass,
      gulp.series("build:styles", "minify:styles", "sync")
    );
  })
);
...
gulp.task("build:demo", function () {
  ...
  var shellCommand = `bundle exec jekyll build --source=${
    paths.src + paths.demo
  } --destination=${paths.demo} --config _config.yml`;
...
gulp.task(
  "demo:dependencies",
  gulp.series("build:demo", function () {
    browserSync.notify("Updating Demo dependencies");
    return gulp
      .src(paths.demo + paths.pattern.html)
      .pipe(
        dependencies({
          src: paths.demo,
          dest: "assets/js",
...
gulp.task(
  "build:styles:watch",
  gulp.series("build:styles", function (callback) {
    browserSync.reload();
    callback();
  })
);
...
gulp.task(
  "build:demo:watch",
  gulp.series("demo:dependencies", function (callback) {
    browserSync.reload();
    callback();
  })
);
...
gulp.task(
  "launch:demo",
  gulp.series("demo:dependencies", function () {
    browserSync.init({
      server: paths.demo,
      ghostMode: false, // Toggle to mirror clicks, reloads etc. (performance)
      logFileChanges: true,
    ...
    gulp.watch("src/demo/**/*.css", gulp.series("minify:styles"));
    ...
    gulp.watch(
      "src/demo/**/*.+(html|md|markdown|MD)",
      gulp.series("build:demo:watch")
    );
